...
GTF_FILE = (
    ...
    / "pan_troglodytes"
    / "release-115"
    / "Pan_troglodytes.Pan_tro_3.0.115.chr.gtf.gz"
)
...
OUTPUT_FILE = (
    OUTPUT_DIR
    / "pan_troglodytes_Pan_tro_3.0_115_chr_protein_coding_with_joshi_hk_status.tsv"
)

UNMAPPED_HK_FILE = (
    OUTPUT_DIR
    / "pan_joshi_2022_hk_ids_not_found_in_chr115_protein_coding.tsv"
)

SUMMARY_FILE = (
    OUTPUT_DIR
    / "pan_joshi_chr115_mapping_summary.tsv"
)
...
def normalise_pan_gene_id(gene_id):
    """
    Remove surrounding whitespace and optional version suffixes.
...
def load_hk_gene_ids(hk_list_file):
    """
    Load one Pan troglodytes Ensembl Gene ID per line.
...
def main():
    if not GTF_FILE.exists():
        raise FileNotFoundError(
            f"Pan chromosome GTF not found: {GTF_FILE}"
        )
    ...
    with gzip.open(
        ...
        for line in infile:
            ...
            chromosome_counts[seqname] += 1

            # Retain only genes with the exact Ensembl biotype
            # protein_coding.
    ...
    with open(
        ...
        for gene_id in unmapped_hk_gene_ids:
            writer.writerow([
                gene_id,
                (
                    "Not found among chromosome-restricted "
                    "Ensembl 115 genes with exact biotype "
                    "protein_coding"
                )
            ])
    ...
    summary_rows = [
        (
            "all_gene_rows_in_chr_gtf",
            all_gene_rows
        ),
        (
            "unique_all_gene_ids_in_chr_gtf",
    ...
    print("\nGene counts")
    print(
        f"All gene rows in chromosome GTF: "
        f"{all_gene_rows}"
    )
    print(
        f"Unique gene IDs in chromosome GTF: "
    ...
    print("\nGene biotypes in chromosome GTF")
    for biotype, count in gene_biotype_counts.most_common():
        print(f"{biotype}\t{count}")

    print("\nChromosome counts for all gene rows")
    for chromosome, count in chromosome_counts.most_common():
        print(f"{chromosome}\t{count}")
